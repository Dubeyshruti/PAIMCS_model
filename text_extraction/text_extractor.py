...
def extract_text_from_pdf(pdf_file: str) -> dict:
    ...
    try:
        if lang == 'h':
            images = convert_from_path(pdf_path, dpi=OPTIMIZED_DPI)
            for image in images:
...
def save_progress(processed_pdfs, current_pdf_text_dict, error_pdfs_list):
    """Saves the current progress to disk."""
    progress_data = {"processed_pdfs": list(processed_pdfs), "error_pdfs": error_pdfs_list} # Convert set to list for JSON serializability
...
def load_progress():
    ...
    try:
        with open(PROGRESS_FILE, 'r') as f_progress:
            ...
            error_pdfs_list = progress_data.get("error_pdfs", [])
    ...
    return processed_pdfs, current_pdf_text_dict, error_pdfs_list

if __name__ == "__main__":
    processed_pdfs, pdf_text_dict, ERROR_pdfs = load_progress() # Load saved progress
        ...
        with ProcessPoolExecutor(max_workers=cpu_count()) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(pdf_files_to_process), desc="Extracting text from PDFs"):
                ...
                if partial_result:
                    ...
                    ERROR_pdfs.append(pdf_file) # Error already handled in extract_text_from_pdf, just record filename

                save_progress(processed_pdfs, pdf_text_dict, ERROR_pdfs) # SAVE PROGRESS AFTER EACH PDF


    # Final Save (in case of clean completion) - redundant if saved after every pdf, but good to have.
    save_progress(processed_pdfs, pdf_text_dict, ERROR_pdfs)

    # Final output saving and error reporting (same as before)
    with open(NCERT_TEXT, 'w') as f:
        dump(pdf_text_dict, f, ensure_ascii=False, indent=4)

    if ERROR_pdfs:
        Series(ERROR_pdfs).to_csv('Exceptions_pdfs.csv', index=False, header=['pdfName'])
