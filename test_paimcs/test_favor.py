...
class TestFAVORProjection(unittest.TestCase):
    ...
    def test_weight_shapes(self):
        ...
        x = tf.random.uniform((1, self.num_heads, self.seq_len, self.input_dim), dtype=tf.float16)
        _ = self.layer(x)
    ...
    def test_call_output_shape(self):
        ...
        batch_size = 4
        x = tf.random.uniform((batch_size, self.num_heads, self.seq_len, self.input_dim), dtype=tf.float16)
        output = self.layer(x)
        ...
        self.assertEqual(output.shape, (batch_size, self.num_heads, self.seq_len, self.num_features))
